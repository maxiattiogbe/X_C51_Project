...
class Generator:
    ...
    def extract_features(self, node: Node) -> list[float]:
        features = []
        INVALID_DESCRIPTOR_VALUE = -1

        # --- Tree context ---
        features.append(node.num_molecules)
        features.append(node.num_reactions)
        features.append(len(node.unique_building_block_ids))

        # --- Reaction metadata ---
        if node.construction_log:
            last_step = node.construction_log[-1]
            features.append(last_step['reaction_id'])  # Could one-hot encode later
            features.append(len(last_step['building_block_ids']))
        else:
            features += [INVALID_DESCRIPTOR_VALUE, INVALID_DESCRIPTOR_VALUE]

        # --- Molecule-level features (for main constructed molecule) ---
        if node.num_molecules > 0:
            ...
            if mol:
                ...
                features += [
                    descriptor(mol)
                    for descriptor in rdkit_descriptors
                ]
            else:
                features += [INVALID_DESCRIPTOR_VALUE] * len(rdkit_descriptors)  # Invalid SMILES fallback
        else:
            features += [INVALID_DESCRIPTOR_VALUE] * len(rdkit_descriptors)  # No molecules fallback
